...
final class BriefingService {
    ...
    /// One turn of the Q&A conversation — the user's question and Crux's answer
    ...
    /// A reply Crux has ALREADY drafted for the top reply-needed item — shown
    ...
    /// (gated by `CRUX_DEMO_MODE`). It's a genuine render of the real UI — so the
    ...
    func loadDemo() {
        ...
        if UserDefaults.standard.bool(forKey: "CRUX_DEMO_CHAT") {
            chat = [
    ...
    func draftReply(for m: MailMessage, tone: ReplyTone = .balanced) async -> ReplyDraft {
        ...
        let instructions = """
        You are Crux, drafting a reply ON BEHALF OF the user to an email they received. Output ONLY the reply body — no subject line, no "Dear ...", no signature, no placeholders like [Your name]. Write as a real person would: \(tone.guidance)
    ...
    func summarizeThread(_ messages: [MailMessage], userAddresses: Set<String>) async -> [String] {
        ...
        let instructions = """
        You are Crux. Summarize this email thread for the user as AT MOST 3 short bullets: (1) what the other side wants, (2) what was decided or changed, (3) what — if anything — is owed FROM the user next. Each bullet is plain text, max 14 words, no preamble, no "the thread". Skip a bullet if there's nothing to say for it.
    ...
    func refresh() async {
        ...
        if UserDefaults.standard.bool(forKey: "CRUX_DEMO_MODE") { loadDemo(); return }
        ...
        if UserDefaults.standard.bool(forKey: "AURA_DEBUG_SEED_INTERESTS") {
    ...
    private func structuredBriefing(
        ...
        let instructions = """
        You are Crux, a warm and concise personal assistant. You speak like a thoughtful friend reminding the user what's in their inbox — natural, conversational, never robotic.
