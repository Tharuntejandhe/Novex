...
/// NSPopover) instead of SwiftUI's MenuBarExtra. See `CruxApp` for why: a fixed
...
public extension Notification.Name {
    ...
    static let cruxDidCompleteOnboarding = Notification.Name("CruxDidCompleteOnboarding")
}
...
public final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// The "notification flies to Crux" dot animation.
    ...
    public func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        popover.contentViewController = NSHostingController(rootView: CruxMenuBarPanel())

        // The menu bar icon.
        let item = NSStatusBar.system.statusItem(withLength: NSStatusItem.variableLength)
        if let button = item.button {
            button.image = NSImage(systemSymbolName: "sparkle", accessibilityDescription: "Crux")
        ...
        NotificationCenter.default.addObserver(
            forName: .cruxDidCompleteOnboarding, object: nil, queue: .main
        ) { [weak self] _ in
    ...
    /// Demo (screenshots only): drop a notch card, or loop the fly-to-Crux dot so
    /// a GIF can be captured. Gated by CRUX_DEMO_PEEK / CRUX_DEMO_FLIGHT.
    private func debugDemoNotchIfRequested() {
        if UserDefaults.standard.bool(forKey: "CRUX_DEMO_PEEK") {
            Task { @MainActor in
        ...
        if UserDefaults.standard.bool(forKey: "CRUX_DEMO_FLIGHT") {
            Task { @MainActor in
    ...
    private func installNotchIfNeeded() {
        ...
        controller.install()
        notch = controller
        // Tapping a notch notification flies a dot to the Crux icon, then opens
        // the panel focused on that mail.
        NotchModel.shared.onTap = { [weak self] peek in
            Task { @MainActor in self?.flyNotificationToCrux(peek) }
        }
        startMailWatcher()
    ...
    /// even when Crux's panel is closed (the refresh loop alone only runs while
    ...
    /// The notification → Crux handoff: collapse the peek, fly the dot from the
    /// notch to the menu-bar icon, then open the panel showing that mail.
    private func flyNotificationToCrux(_ peek: NotchModel.PeekItem) {
        ...
        guard animate, let start = notch?.notchAnchor, let end = cruxIconScreenPoint() else {
    ...
    /// Center of the menu-bar Crux icon, in screen coordinates (works on any screen).
    private func cruxIconScreenPoint() -> CGPoint? {
    ...
    func debugShowPopoverIfRequested() {
        guard UserDefaults.standard.bool(forKey: "AURA_DEBUG_SHOW_POPOVER") else { return }
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { [weak self] in
            self?.togglePopover(nil)
            if let q = UserDefaults.standard.string(forKey: "AURA_DEBUG_ASK"), !q.isEmpty {
