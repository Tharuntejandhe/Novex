...
/// One snoozed item: an email hidden from Crux until `wake`.
...
/// from Crux's views until its wake time, then resurfaces it. Fully on-device.
enum SnoozeStore {
    ...
    /// so they leave the store and reappear in Crux. Returns them so the caller
