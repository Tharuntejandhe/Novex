...
///   it themselves, it's theirs. Crux never fights for the window.
enum MailSync {
    ...
    /// or `log stream --predicate 'subsystem == "com.tarun.crux"'`. No files on
    /// disk (the old world-readable /tmp/crux-sync.log is gone), privacy-aware,
    /// and ~free when nobody is listening.
    private static let logger = Logger(subsystem: "com.tarun.crux", category: "sync")
