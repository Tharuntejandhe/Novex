...
struct WidgetView: View {
    ...
    /// The owner's name, captured at setup — Crux is a personal agent, so it
    ...
    private var cruxBackground: some View {
        ZStack {
    ...
    private var onboardingBody: some View {
        ZStack {
            ...
            OnboardingView {
                ...
                NotificationCenter.default.post(name: .cruxDidCompleteOnboarding, object: nil)
                Task {
    ...
    private var widgetBody: some View {
        ...
        .onAppear {
            ...
            if UserDefaults.standard.bool(forKey: "AURA_DEBUG_SHOW_SETTINGS") { showSettings = true }
            // Demo (screenshots): open on a chosen tab.
            if let raw = UserDefaults.standard.string(forKey: "CRUX_DEMO_TAB"),
    ...
    private var stateContent: some View {
        ...
        case .needsFullDiskAccess:
            actionCard(
                ...
                detail: "Crux needs to read Mail.app's local store",
    ...
    /// The Q&A conversation — your question echoed as a bubble, Crux's reply
    /// below it, with history. A "Back" returns to the briefing.
    private var chatView: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Text("Ask Crux").font(.system(size: 11, weight: .semibold))
                    .foregroundStyle(.white.opacity(0.5))
                Spacer()
    ...
    private func chatTurn(_ turn: BriefingService.ChatTurn) -> some View {
        ...
        // Crux's answer — left-aligned, with a sparkle.
    ...
    /// A gentle "a new Crux is out" card at the top of the briefing (only when the
        ...
        if let up = updater.available {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 1) {
                    Text("Crux \(up.version) is available")
    ...
    /// "On your plate" — open Apple Reminders due/overdue, so Crux is aware of
    ...
    private func primaryHelp(_ item: BriefingItem) -> String {
        if item.action == .reply, service.message(forID: item.messageID) != nil {
            return "Draft a reply with Crux"
        }
    ...
    /// Tappable prompts that make Crux feel like an assistant you talk to, not a
