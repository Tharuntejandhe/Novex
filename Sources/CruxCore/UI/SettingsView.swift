...
struct SettingsView: View {
        ...
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text("Check for updates")
                    .font(.system(size: 12, weight: .semibold))
                    .foregroundStyle(.white.opacity(0.92))
                Text("Once a day, anonymously, via GitHub — the only time Crux uses the network")
                    .font(.system(size: 10))
                    .foregroundStyle(.white.opacity(0.5))
    ...
    // MARK: - What Crux learned about you
    ...
    private var mutedSection: some View {
        VStack(alignment: .leading, spacing: 7) {
            label("MUTED SENDERS", hint: "Hidden across Crux")
    ...
    private var aboutSection: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            HStack(spacing: 8) {
                Text("Crux · on-device, private · v0.1")
    ...
    private func sendFeedback() {
        ...
        let subject = "Crux feedback".addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ""
