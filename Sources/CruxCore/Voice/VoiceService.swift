...
final class VoiceService {
    ...
    func ensureAuthorized() async -> Bool {
        NSLog("[Crux.Voice] ensureAuthorized: starting")
        ...
        NSLog("[Crux.Voice] speech auth status = \(speechAuth.rawValue)")
        ...
        NSLog("[Crux.Voice] mic granted = \(micGranted)")
    ...
    func startRecording() async throws {
        NSLog("[Crux.Voice] startRecording called")
        guard await ensureAuthorized() else {
            NSLog("[Crux.Voice] not authorized — bailing")
            return
        }
        guard let recognizer = recognizer, recognizer.isAvailable else {
            NSLog("[Crux.Voice] recognizer unavailable (locale: \(recognizer?.locale.identifier ?? "nil"))")
            state = .error("Speech recognizer not available for this locale")
            throw NSError(domain: "Crux.Voice", code: 1)
        }
        NSLog("[Crux.Voice] recognizer ready, locale=\(recognizer.locale.identifier)")
        ...
        do {
            ...
            NSLog("[Crux.Voice] audio engine started successfully")
        } catch {
            NSLog("[Crux.Voice] audio engine start failed: \(error)")
        ...
        state = .recording
        NSLog("[Crux.Voice] state = recording")
